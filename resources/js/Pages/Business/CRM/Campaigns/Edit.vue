...
const form = useForm({
    ...
    segment_id: props.campaign.segment_id ?? null,
    promotion_id: props.campaign.promotion_id ?? null,
    content_html: props.campaign.content_html || '',
    content_text: props.campaign.content_text || '',
});

const submit = () => {
    form.put(`/business/crm/campaigns/${props.campaign.id}`);
};
</script>
    ...
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="flex items-start justify-between gap-4 mb-6">
            <div>
                ...
                <p class="text-gray-400 text-sm mt-1">Update your draft before sending.</p>
            </div>
            <Link :href="`/business/crm/campaigns/${campaign.id}`" class="px-4 py-2 rounded-lg bg-white/10 text-gray-200 hover:bg-white/20 border border-white/10 text-sm">
                Back
            </Link>
        </div>
