...
const form = useForm({
    ...
    punches_required: 10,
    reward_value: null,
    punch_card_max_cards_per_user: null,
    punch_card_total_cards_limit: null,
    punch_card_max_punches_per_day: null,
    punch_icon: '⭐',
    tiers: [],
    original_price: null,
    ...
    switch (form.discount_type) {
        ...
        case 'first_time':
            return form.discount_value
                ? (form.rules.first_time_kind === 'fixed'
                    ? `Welcome: $${form.discount_value} Off`
                    : `Welcome: ${form.discount_value}% Off`)
                : 'Welcome Offer';
        default:
...
watch(() => form.discount_type, (newVal) => {
    ...
    } else if (newVal === 'first_time') {
        form.rules.first_time_kind = form.rules.first_time_kind || 'percentage';
    }
});
...
// Submit
const submit = () => {
    form.post(route('business.promotions.store'));
};
...
const applyTemplate = (template) => {
    ...
    // Core
    if (typeof template.name === 'string') form.name = template.name;
    if (typeof template.description === 'string') form.description = template.description;
    if (typeof template.terms === 'string') form.terms = template.terms;
    if (typeof template.discount_type === 'string') form.discount_type = template.discount_type;

    // Values
    if (template.discount_value !== undefined) form.discount_value = template.discount_value;
    if (template.buy_quantity !== undefined) form.buy_quantity = template.buy_quantity;
    if (template.get_quantity !== undefined) form.get_quantity = template.get_quantity;
    if (template.for_price !== undefined) form.for_price = template.for_price;
    if (template.punches_required !== undefined) form.punches_required = template.punches_required;
    if (template.reward_value !== undefined) form.reward_value = template.reward_value;
    if (template.minimum_purchase !== undefined) form.minimum_purchase = template.minimum_purchase;
    if (template.maximum_discount !== undefined) form.maximum_discount = template.maximum_discount;
    if (template.original_price !== undefined) form.original_price = template.original_price;
    if (template.punch_icon !== undefined) form.punch_icon = template.punch_icon;

    // Tiered
    if (Array.isArray(template.tiers)) form.tiers = template.tiers;

    // Rules (merge known keys only)
    if (template.rules && typeof template.rules === 'object') {
        const r = template.rules;
        if (r.max_redemptions_total !== undefined) form.rules.max_redemptions_total = r.max_redemptions_total;
        if (r.max_redemptions_per_user !== undefined) form.rules.max_redemptions_per_user = r.max_redemptions_per_user;
        if (r.max_per_day !== undefined) form.rules.max_per_day = r.max_per_day;
        if (r.max_per_week !== undefined) form.rules.max_per_week = r.max_per_week;
        if (r.max_per_month !== undefined) form.rules.max_per_month = r.max_per_month;
        if (Array.isArray(r.valid_days)) form.rules.valid_days = r.valid_days;
        if (r.valid_hours && typeof r.valid_hours === 'object') {
            if (r.valid_hours.start !== undefined) form.rules.valid_hours.start = r.valid_hours.start;
            if (r.valid_hours.end !== undefined) form.rules.valid_hours.end = r.valid_hours.end;
        }
        if (r.combinable !== undefined) form.rules.combinable = !!r.combinable;
        if (Array.isArray(r.buy_item_prices)) form.rules.buy_item_prices = r.buy_item_prices;
        if (Array.isArray(r.get_item_prices)) form.rules.get_item_prices = r.get_item_prices;
    }

    // Dates
    if (template.starts_at !== undefined) form.starts_at = template.starts_at;
    if (template.ends_at !== undefined) form.ends_at = template.ends_at;
};

// Handle type/template query parameter from URL
onMounted(() => {
    const urlParams = new URLSearchParams(window.location.search);
    const type = urlParams.get('type');
    if (type && discountTypeCards.some(card => card.type === type)) {
    ...
    if (templateParam) {
        try {
            ...
            // Skip the type-selection step when coming from a template.
            step.value = 2;
            // Clean URL so refresh doesn't re-apply / and remove huge querystring.
            window.history.replaceState({}, document.title, '/business/promotions/create');
        } catch (e) {
...
<template>
    ...
    <div class="max-w-4xl mx-auto px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div class="mb-8">
            ...
            <p class="text-gray-400 mt-1">Build your perfect discount or deal</p>
        </div>
        ...
        <div class="glass-card p-8">
                ...
                <div class="p-4 rounded-xl bg-white/5">
                    ...
                    <div v-if="form.discount_type !== 'punch_card'" class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <p class="text-xs text-gray-500 mt-1">Maximum number of times this promotion can be redeemed across all customers combined.</p>
                        </div>
                        <div>
            ...
            <div v-show="step === 4" class="space-y-6">
                <h2 class="text-xl font-semibold text-white mb-4">Review Your Promotion</h2>
                ...
                <div class="flex items-center justify-between p-4 rounded-xl bg-white/5">
                    <div>
                        <p class="text-white font-medium">Activate Immediately</p>
                        <p class="text-gray-400 text-sm">Make this promotion live after creation</p>
                    </div>
                    <button
